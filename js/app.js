...
function onSearch(q){
  const qv = (q || '').trim();
  document.getElementById('sx').style.display=qv?'block':'none';
  let res=KAMUS;
...
// Default 4-band yang wajar (1kΩ ±5%) — hindari render awal 0Ω yang menyesatkan
let resistSel = [1,0,2,10]; // Coklat(1), Hitam(0), Merah(×100), Emas(±5%) → 1 kΩ
...
function initSkema(){
  ...
  // Halaman skema saat ini memakai AI Diagram (mermaid). Jika elemen #skema-list
  // tidak ada (mis. konten interaktif lama dihapus), jangan crash — lewati saja.
  if(!list) return;
...
window.onload=()=>{
  ...
  renderChips();
  // Lewati onSearch('') agar label + jumlah istilah (scount) ikut terisi saat load pertama
  onSearch('');
  initQuiz();
  initCalc();
  initSkema();
...
// ═══════════════════════════════════════════════════════════
// VIDEO PEMBELAJARAN LOGIC
// Daftar statis channel YouTube Indonesia (lihat VIDEOS di data.js)
// ═══════════════════════════════════════════════════════════
let videoTopic = 'Semua';

function videoChips() {
  const wrap = document.getElementById('video-chips');
  if (!wrap || typeof VIDEO_TOPICS === 'undefined') return;
  wrap.innerHTML = VIDEO_TOPICS.map(t =>
    `<button class="video-chip${t === videoTopic ? ' on' : ''}" onclick="filterVideos('${t}')">${t}</button>`
  ).join('');
}

function filterVideos(t) {
  videoTopic = t;
  videoChips();
  renderVideos();
}
...
function renderVideos(shuffle = false) {
  ...
  let list = VIDEOS.filter(v => videoTopic === 'Semua' || v.topic === videoTopic);
  if (shuffle) list = [...list].sort(() => Math.random() - 0.5);

  if (count) {
    count.textContent = `${list.length} video pilihan · channel YouTube Indonesia`;
  }

  if (list.length === 0) {
    grid.innerHTML = `<div class="empty" style="display:block">Tidak ada video untuk topik ini.</div>`;
    ...
    return `
      <div class="news-card" style="animation-delay: ${Math.random() * 0.5}s">
        <div class="vid-thumb-wrap" onclick="openVideoById('${v.id}')" role="button" tabindex="0"
          ...
          <span class="vid-tag">${v.topic}</span>
        </div>
        <div class="news-body">
          ...
          <div class="news-actions">
            ...
            <a href="${v.channelUrl}" target="_blank" rel="noopener" class="news-btn g-trans">
              <span>📺 Channel</span>
            </a>
          </div>
        </div>
...
function openVideo(v) {
  ...
  if (ext) ext.href = 'https://www.youtube.com/watch?v=' + v.id;
...
function closeVideo() {
  ...
  if (frame) frame.src = ''; // hentikan pemutaran
}
